...
class HybridRetrievalIntegrationTests {
    private static final String RUNTIME_LOGIN = "nexora_hybrid_runtime_login";
    private static final String RUNTIME_PASSWORD = "test-hybrid-runtime-login";
    ...
    private static final PostgreSQLContainer<?> DATABASE = new PostgreSQLContainer<>("pgvector/pgvector:0.8.1-pg17")
            .withDatabaseName("nexora_hybrid")
    ...
    static void properties(DynamicPropertyRegistry registry) {
        ...
        registry.add("nexora.realtime.descriptor.jwt-secret", () -> "test-hybrid-realtime-secret-for-m4-t04");
    }
    ...
    @Test
    void retrievesAuthorizedChunksAcrossBothBranchesAndDeniesCrossTenant() throws Exception {
        TenantFixture tenant = seedTenant("Hybrid");
        TenantFixture other = seedTenant("HybridOther");

        UUID docId = UUID.randomUUID();
        UUID chunkLexical = UUID.randomUUID();
        UUID chunkVector = UUID.randomUUID();
        seedDocumentAndChunk(tenant, docId, chunkLexical, "Immutable publishing rolls back by creating a new version.");
        seedDocumentAndChunk(tenant, docId, chunkVector, "The theme engine serializes tokens into bounded manifests.");

        vectors.embedAndStore(tenant.ownerContext(), chunkLexical);
        vectors.embedAndStore(tenant.ownerContext(), chunkVector);

        HybridRetrievalService.RetrievalResult lexical = retrieval.retrieve(tenant.ownerContext(), "publishing rollback");
        assertThat(lexical.fusionVersion()).isEqualTo("nexora-rrf-v1");
        assertThat(lexical.matches()).isNotEmpty();
        assertThat(lexical.matches()).extracting(HybridRetrievalService.FusedMatch::chunkId)
                .contains(chunkLexical);

        HybridRetrievalService.RetrievalResult crossTenant = retrieval.retrieve(other.ownerContext(), "publishing rollback");
        assertThat(crossTenant.matches()).isEmpty();
    }
    ...
    private void seedDocumentAndChunk(TenantFixture tenant, UUID documentId, UUID chunkId, String text) throws Exception {
        try (Connection connection = DriverManager.getConnection(
            ...
            statement.execute("INSERT INTO nexora.knowledge_bases (id, organization_id, name, description, state, created_by_subject_id) VALUES ('"
                    + tenant.knowledgeBaseId() + "', '" + tenant.organizationId() + "', 'Hybrid KB', '', 'ACTIVE', '"
            ...
            statement.execute("INSERT INTO nexora.documents (id, knowledge_base_id, organization_id, original_name, stored_object_key, content_type, byte_size, sha256, state, uploaded_by_subject_id) VALUES ('"
                    ...
                    + "', 'hybrid-doc.txt', 'k/hybrid-doc', 'text/plain', 128, '"
    ...
    private TenantFixture seedTenant(String label) throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(
            ...
            statement.execute("INSERT INTO nexora.organizations (id, slug, name, owner_membership_id) VALUES ('"
                    ...
                    + "', 'Hybrid " + label + "', '" + ownerMembershipId + "')");
    ...
    private static void prepareRuntimeRole() {
        try (Connection connection = DriverManager.getConnection(
            ...
            throw new IllegalStateException("Unable to prepare the hybrid runtime role", exception);
        }
    }

    private static void prepareMigrations() {
        try {
            migrationDirectory = Files.createTempDirectory("nexora-hybrid-flyway-");
            ...
            throw new IllegalStateException("Unable to prepare hybrid migrations", exception);
        }
    }
