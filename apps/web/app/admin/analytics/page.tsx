...
export default function AnalyticsPage() {
  ...
  return <AppShell><PageGrid><main className="nx-admin-page">
    <p className="nx-eyebrow">Analytics</p>
    <h1 ref={heading} tabIndex={-1}>Product analytics</h1>
    <p className="nx-lede">Event volume by type for the selected time window.</p>

    {state === "loading" && <section className="nx-access-card" role="status"><StatusLabel kind="loading" /> Loading analytics…</section>}
    {state === "empty" && <section className="nx-access-card"><StatusLabel kind="planned" /><h2>Choose an organization</h2><p>Select an organization in your account to view its analytics.</p></section>}
    ...
    {state === "ready" && <section className="nx-access-card">
      <div className="nx-card-heading">
        <div><h2>Event breakdown</h2><p className="nx-field-help">{totalEvents} total events · last {days} days</p></div>
        <StatusLabel kind="fixture" />
      </div>
      <div className="nx-analytics-controls">
        {[7, 14, 30].map((d) => (
          <ActionButton key={d} tone={days === d ? "primary" : "secondary"} onClick={() => setDays(d)}>{d}d</ActionButton>
        ))}
      </div>
      {aggregation.length === 0 ? <p className="nx-empty-copy">No events recorded in this window.</p> :
